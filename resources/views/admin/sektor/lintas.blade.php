        ...
        <div class="box box-warning">
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <a href="{{ route('survei.sektor-ekonomi', [Crypt::encrypt($survei->id), Crypt::encrypt(4)]) }}" class="btn btn-warning btn-sm"><i class="fa fa-backward"></i>&nbsp; Kembali</a>
                </div>
            </div>
            <div class="box-body">
                <div class="row">
                    <div class="col-md-12">
                                        ...
                                        @foreach ($iSektor->pemulihan_sektor as $item)
                                            <tr>
                                                ...
                                                @for ($i = $yearSurveiMin1; $i <= $currentYear; $i++)
                                                    ...
                                                    <td class=""><input type="text"  id="{{ $survei->id }}{{ $item->id }}{{ $i }}" data-survei_id="{{ $survei->id }}" data-kuesioner_id="{{ $item->id }}" data-tahun="{{ $i }}" class="form-control nilai" value="{{ count($detail_survei->where('kuesioner_id', $item->id)->where('tahun', $i)) > 0 ? $detail_survei->where('kuesioner_id', $item->id)->where('tahun', $i)->first()->nilai : ''}}"></td>
                                                @endfor
                                            </tr>
                                        @endforeach
                                    @endforeach
                        ...
                        <div class="box-footer pull-right">
                            <a href="{{ route('calculate', [Crypt::encrypt($survei->id)]) }}" class="btn btn-success">Finish</a>
                        </div>
                    </div>
                </div>
            </div>
