        ...
        <div class="box box-warning">
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <a href="{{ route('survei.sektor-infrastruktur', [Crypt::encrypt($survei->id), Crypt::encrypt(2)]) }}" class="btn btn-warning btn-sm"><i class="fa fa-backward"></i>&nbsp; Kembali</a>
                </div>
            </div>
            <div class="box-body">
                <div class="row">
                    <div class="col-md-12">
                        <form id="form_pemukiman" action="{{ route('sektor-infrastruktur.store',  [Crypt::encrypt($survei->id), Crypt::encrypt(4), 'sektor-ekonomi']) }}" method="post">
                            @csrf
                            @method('PUT')
                                            ...
                                            @foreach ($iSektor->pemulihan_sektor as $item)
                                                <tr>
                                                    ...
                                                    <input type="hidden" name="tahun[]" value="{{ $i }}">
                                                        <td class=""><input type="text" name="nilai[]" id="{{ $survei->id }}{{ $item->id }}{{ $i }}" data-survei_id="{{ $survei->id }}" data-kuesioner_id="{{ $item->id }}" data-tahun="{{ $i }}" class="form-control nilai" value="{{ count($detail_survei->where('kuesioner_id', $item->id)->where('tahun', $i)) > 0 ? $detail_survei->where('kuesioner_id', $item->id)->where('tahun', $i)->first()->nilai : ''}}"></td>
                                                    @endfor
                                                </tr>
                                            @endforeach
                                        @endforeach
